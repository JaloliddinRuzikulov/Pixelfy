...
export async function POST(request: NextRequest) {
	try {
		...
		console.log("📤 Submitting TTS job to lipsync service:", lipsyncServiceUrl);
		console.log("🔑 Using API key:", apiKey.substring(0, 10) + "...");

		// Build API URL for job submission with TTS
		const apiUrl = lipsyncServiceUrl.includes('/lipsync')
			? `${lipsyncServiceUrl}/submit-job-from-text`
			: `${lipsyncServiceUrl}/lipsync/submit-job-from-text`;
		...
		console.log("✅ TTS Job submitted successfully:", data.job_id);

		return NextResponse.json(data);
	} catch (error) {
		console.error("❌ Error in submit-job-from-text API:", error);
