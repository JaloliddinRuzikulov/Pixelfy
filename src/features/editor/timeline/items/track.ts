...
class Track extends TrackBase {
	...
	static createResizeControls() {
		return {
			controls: {
				// Vertikal resize for track height
				mt: new Control({
					x: 0,
					y: -0.5,
					...
					actionName: 'resizing',
					sizeX: 100,  // Wide area for easier selection
					sizeY: 5,    // Thin height for resize handle
					offsetY: -2, // Offset to position above track
				}),
				mb: new Control({
					x: 0,
					y: 0.5,
					...
					sizeX: 100,  // Wide area for easier selection
					sizeY: 5,    // Thin height for resize handle
					offsetY: 2,  // Offset to position below track
				}),
			},
		};
	}
	...
	constructor(props: TrackItemProps) {
		super(props);
		console.log("Track constructor called with props:", props);
		this.fill = "#18181b";
		this.items = props.items || [];
		this.magnetic = props.magnetic || false;
		...
		this.controls = Track.createResizeControls().controls;
		this.hasControls = true;
		this.selectable = true;

		console.log("Track controls enabled:", {
		...
		const minScaleY = this.height > 0 ? this.minHeight / this.height : 1;
		const maxScaleY = this.height > 0 ? this.maxHeight / this.height : 1;

		this.set({
			minScaleX: 1,  // Prevent horizontal scaling
			maxScaleX: 1,  // Prevent horizontal scaling
			minScaleY: minScaleY,
			maxScaleY: maxScaleY,
		...
		console.log("Track resize constraints set:", {
			minScaleY: this.minHeight / this.height,
			maxScaleY: this.maxHeight / this.height
		});
	}

	// Override scaling behavior
	public _constrainScale(value: any) {
		console.log("Track _constrainScale called with value:", value);

		// Ensure value is valid
		if (!value || typeof value !== 'object') {
			console.log("Invalid scale value, creating default");
			value = { x: 1, y: 1 };
		}

		// Call parent constrainScale if available
		const result = super._constrainScale?.(value) || { ...value };

		console.log("Super _constrainScale result:", result);

		// Ensure height is set before calculating constraints
		if (this.height > 0) {
			const minScaleY = this.minHeight / this.height;
			const maxScaleY = this.maxHeight / this.height;

			// Constrain height scaling
			if (result.y < minScaleY) {
				console.log("Constraining to minHeight:", minScaleY);
				result.y = minScaleY;
			}
			if (result.y > maxScaleY) {
				console.log("Constraining to maxHeight:", maxScaleY);
				result.y = maxScaleY;
			}
		}
		...
		console.log("Final constrained result:", result);
		return result;
	}

	// add custom text to the track item
	public _render(ctx: CanvasRenderingContext2D) {
		console.log("Track _render called", { width: this.width, height: this.height, fill: this.fill });

		// Call parent render if available
		if ("_render" in TrackBase.prototype) {
			(TrackBase.prototype as any)._render.call(this, ctx);
		}

		// Render track background with visible border
		ctx.save();
		...
		if (this.active) {
			...
			ctx.fillRect(0, this.height - 2, this.width, 4);
		}
