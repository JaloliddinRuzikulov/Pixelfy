...
function simulateEditorLoad() {
	...
	setTimeout(() => {
		...
		const mediaButtons = document.querySelectorAll(
			'[data-testid*="media"], button[class*="media"], button[class*="video"], button[class*="image"]',
		);
		console.log("📺 Media buttons found:", mediaButtons.length);

		if (mediaButtons.length > 0) {
			console.log(
				"🖱️ Attempting to click first media button to add to timeline",
			);
			...
			console.log(
				"⚠️ No media buttons found, checking for other ways to add media",
			);
			checkForTracks();
		}
	}, 1000);
...
function checkResizeArea(x, y) {
	...
	simulatedTracks.forEach((track, index) => {
		...
		const nearTopEdge =
			Math.abs(y - trackTop) <= 5 && x >= trackLeft && x <= trackRight;
		const nearBottomEdge =
			Math.abs(y - trackBottom) <= 5 && x >= trackLeft && x <= trackRight;
		const nearLeftEdge =
			Math.abs(x - trackLeft) <= 5 && y >= trackTop && y <= trackBottom;
		const nearRightEdge =
			Math.abs(x - trackRight) <= 5 && y >= trackTop && y <= trackBottom;

		if (nearTopEdge || nearBottomEdge) {
			console.log(
				`🔄 Near vertical resize edge of ${track.name} (${nearTopEdge ? "top" : "bottom"})`,
			);
		}

		if (nearLeftEdge || nearRightEdge) {
			console.log(
				`🔄 Near horizontal resize edge of ${track.name} (${nearLeftEdge ? "left" : "right"})`,
			);
		}
